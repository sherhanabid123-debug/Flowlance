...
export default function DashboardOverview() {
    ...
    clients.forEach(c => {
      ...
      if (y === currentYear) {
        ...
        if (c.shares && c.shares.length > 0) {
          const myShare = c.shares.find((s: any) => getUID(s.userId) === myId);
          if (myShare) {
            personalAmount = (c.finalAmount * myShare.percentage) / 100;
          }
        } else {
          // Default: Owner gets 100% if no shares defined
          const ownerIdString = getUID(workspace?.ownerId);
          if (ownerIdString === myId) {
            personalAmount = (c.finalAmount || 0);
          }
        }
        
        timeline[m].sales += personalAmount;
      }
    });
  ...
  const isOwner = user?._id && workspace?.ownerId && (user._id === (workspace.ownerId._id || workspace.ownerId).toString());

  const statCards = [
    { 
      title: isOwner ? 'Total Revenue' : 'Team Revenue', 
      value: `₹${metrics.totalSales.toLocaleString('en-IN')}`, 
      icon: Wallet, 
      color: 'text-white', 
      bg: 'bg-green-600 shadow-lg shadow-green-600/10', 
      tooltip: isOwner ? 'Total actual income from completed projects' : 'Total revenue generated by the entire team' 
    },
    { 
      title: 'My Earnings', 
      value: `₹${metrics.myEarnings.toLocaleString('en-IN')}`, 
      icon: CheckCheck, 
      color: 'text-white', 
      bg: 'bg-indigo-600 shadow-lg shadow-indigo-600/10', 
      tooltip: 'Your personal share of revenue across all completed projects' 
    },
    { 
      title: 'Potential Revenue', 
      value: `₹${metrics.potentialRevenue.toLocaleString('en-IN')}`, 
      icon: TrendingUp, 
      color: 'text-white', 
      bg: 'bg-blue-600 shadow-lg shadow-blue-600/10', 
      tooltip: 'Total estimated revenue from potential clients' 
    },
    { 
      title: 'Total Clients', 
      ...
      icon: Users, 
      color: 'text-white', 
      bg: 'bg-black/40 shadow-lg', 
      tooltip: 'Sum of all clients in your database' 
    },
    { 
      ...
      icon: Target, 
      color: 'text-black', 
      bg: 'bg-amber-500 shadow-lg shadow-amber-500/10', 
      tooltip: 'Currently active (confirmed) projects' 
    },
  ];
